...
public class GameMap extends JPanel {
    ...
    private void showMessageGameOver(Graphics g) {
        ...
        g.setFont(new Font("TimesNewRoman", Font.BOLD, 46));
        FontMetrics metrics = g.getFontMetrics(new Font("TimesNewRoman", Font.BOLD, 46));
        String text = "";
        switch (stateGameOver) {

            case STATE_DRAW:
                text = "Победила дружба!";
                g.drawString(text, (getWidth() - metrics.stringWidth(text)) / 2, getHeight() / 2 + metrics.getHeight() / 2);
                break;
            case STATE_WIN_HUMAN:
                text = "Ура! Вы победили!!!";
//                g.drawString("HUMAN Wins!", getWidth()/2 - 6, getHeight()/ 2 + 30);
                g.drawString(text, (getWidth() - metrics.stringWidth(text)) / 2, getHeight() / 2 + metrics.getHeight() / 2);
                break;
            case STATE_WIN_AI:
                text = "Победа робота!!!";
//                g.drawString("AI Wins!", getWidth()/2 - 4, getHeight()/ 2 + 30);
                g.drawString(text, (getWidth() - metrics.stringWidth(text)) / 2, getHeight() / 2 + metrics.getHeight() / 2);
                break;
        }
    ...
    private void aiHuman(MouseEvent e){
        int cellX = e.getX() / cellWidth;
        int cellY = e.getY() / cellHeight;
        System.out.printf("\nHuman  Y = "+ gameMode );
        // Ошибка была в !isCellEmpty(celly, cellx)
        if (!isCellValid(cellY, cellX) || !isCellEmpty(cellX, cellY)) return;
        //System.out.printf("\nHuman  Y = "+ cellY + " X = "+ cellX + " DOT " + field[cellY][cellX] + " isCellValid(cellY, cellX) " + !isCellValid(cellY, cellX) + " !isCellEmpty(cellY, cellX) " + !isCellEmpty(cellY, cellX));
        field[cellY][cellX] = DOT_HUMAN;
        repaint();
        if (gameCheck(DOT_HUMAN, STATE_WIN_HUMAN)) return;
    }
    private void human_2Turn(MouseEvent e){
